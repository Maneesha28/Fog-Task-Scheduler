...
public class Main {
    ...
    public static void runMultiple() {
        ...
        for (File file : listOfFiles) {
            if (file.isFile()) {
                ...
                runSingleSimulation(fileName, Integer.parseInt(parts[2]));
    ...
    public static void runSingleSimulation(String csvFileName, int itr) {
        //create 3 vms
        Vm highResourceVm = new Vm(0, 5833);
        Vm midResourceVm = new Vm(1, 5833);
        Vm lowResourceVm = new Vm(2, 5833);
        //create a host
        Host host = new Host(highResourceVm, midResourceVm, lowResourceVm);
        ...
        System.out.println("Submitted tasks to host");
    ...
    public static void writeToFile(List<Task> tasks, int itr){
        ...
        String outputFilename = outputDir + "\\CustomThreeHomogeneousVM\\result_" + itr + ".csv";
        try {
            ...
            csvWriter.flush();
            csvWriter.close();
            System.out.println("Results written to homogeneous_algo_results.csv");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
