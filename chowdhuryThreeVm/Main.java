...
public class Main {
    ...
    public static void runSingleSimulation(String csvFileName, int itr) {
        Vm highResourceVm = new Vm(0, 10000);
        Vm midResourceVm = new Vm(1, 5000);
        Vm lowResourceVm = new Vm(2, 2500);
        ...
        int highQLimit = (int) (tasks.size()*0.2);
        int midQLimit = (int) (tasks.size()*0.3);
        Host host = new Host(highResourceVm, midResourceVm, lowResourceVm, highQLimit, midQLimit);
    ...
    public static void writeToFile(List<Task> tasks, int itr) {
        ...
        String outputFilename = outputDir + "\\ChowdhuryThreeVM\\result_" + itr + ".csv";
        ...
        try {
            ...
            csvWriter.flush();
            csvWriter.close();
            System.out.println("Results written to chowdhury_algo_results.csv");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    public static void main(String[] args) { runMultiple(); }
}